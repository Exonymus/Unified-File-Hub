...
public class RepositoryController {
    ...
    @GetMapping("/")
    public String getPersonalPage(@RequestParam(value = "publicationName", required=false) String publicationName,
        ...
        List<File> files = fileStorageService.getFilesByUser(authentication.getName());
    ...
    @PostMapping("/file_control")
    public String controlFile(@RequestParam(value = "openfile", required=false) String openfile,
                              @RequestParam(value = "delfile", required=false) String delfile,
                              @RequestParam(value = "editfile", required=false) String editfile) {
        if(openfile != null)
            return "redirect:/openfile/?filename=" + openfile;
        else if (editfile != null)
            return "redirect:/editfile/?filename=" + editfile;
        else if (delfile != null)
            fileStorageService.deleteFile(delfile);
        return "redirect:/";
    }
}