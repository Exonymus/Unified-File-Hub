from sqlalchemy import MetaData, Table, select, Engine, or_, and_
...
class DatabaseController:
    ...
    def create_file(self, download_id, filename, author_name, publication_name, theme,
        ...
        SessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=self.engine)
        session = SessionLocal()
        file = Files(download_id=download_id, filename=filename, author_name=author_name,
                     publication_name=publication_name,
                     theme=theme, publication_date=publication_date, description=description, upload_date=upload_date,
                     uploader_name=uploader_name,
                     doc_type=doc_type, is_public=bool(int(is_public)), data=input_data.file.read(),
    ...
    def copy_file(self, file_id, folder_path, username):
        ...
        copy_file = Files(filename=str(uuid.uuid4()) + file.publication_name, download_id=str(uuid.uuid4()),
                          ...
                          uploader_name=username, doc_type=file.doc_type, is_public=0, data=file.data,
                          folder_path=folder_path)
    ...
    def update_file(self, file_id, author_name, publication_name, theme, publication_date, description, is_public,
                    folder_path):
        SessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=self.engine)
        session = SessionLocal()
        file = session.query(Files).filter(Files.id == file_id).first()
